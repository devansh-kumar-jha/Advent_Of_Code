...
int next(vector<string>& mat,int& i,int& j,int n,int m,int dir) {
    ...
    if(dir==0) { do{ j=(j+1)%m; }while(mat[i][j]=='0'); }
    else if(dir==1) { do{ i=(i+1)%n; }while(mat[i][j]=='0'); }
    else if(dir==2) { do{ j=(j+m-1)%m; }while(mat[i][j]=='0'); }
    else { do{ i=(i+n-1)%n; }while(mat[i][j]=='0'); }
