...
void round(vector<queue<int>>& monkeys,vector<vector<int>>& ops,vector<vector<int>>& test,vector<int>& nums,int round)
{
    int n=monkeys.size();
    ...
    if(round%1000==0) { { for(auto it:nums) cerr<<it<<" "; } cerr<<"\n"; }
    return;
}
...
int32_t main()
{
    ...
    for(int i=0;i<test.size();i++) mod=mod*test[i][0];
    vector<int> nums(monkeys.size(),0);
    for(int i=0;i<10000;i++) round(monkeys,ops,test,nums,i+1);
    // for(auto it:monkeys) cerr<<it.size()<<" "; cerr<<"\n";
