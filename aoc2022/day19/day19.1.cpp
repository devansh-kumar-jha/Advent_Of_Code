...
// a - ore's for ore robot, b - ore's for clay robot
// c - ore and clay for obsidian robot
// d - ore and obsidian for geode robot
int calculator_max(int a,int b, pair<int,int> c, pair<int,int> d) {
    // cout<<a<<" "<<b<<" "<<c.first<<" "<<c.second<<" "<<d.first<<" "<<d.second<<"\n";
    int ans=0,ore=0,clay=0,obs=0,geode=0;
...
int32_t main()
{
    ...
    while(getline(fin, s)) {
        ...
        int num=calculator_max(a,b,c,d);
        ans+=(ctr*num); ctr++;
        cout<<num<<"\n";
    }
    cout<<ans<<"\n";
